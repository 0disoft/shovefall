...
describe("content description SSOT", () => {
  it("derives every item description from its definition", () => {
    const expected = {
      ...
      soap: "4회 · 내 위치에서 최대 3칸 떨어진 곳에 비누 설치 · 밟은 상대는 진행하던 방향으로 2초 동안 길게 미끄러짐 · 미끄러짐이 끝나면 피해 30 · 피해를 받은 뒤 1.5초 기절 · 내가 설치한 비누에는 미끄러지지 않음",
      "brick-bag":
  ...
  it("separates card metadata from the effect copy without duplicating charges", () => {
    ...
    expect(formatItemEffectDescription(soap)).toBe(
      "내 위치에서 최대 3칸 떨어진 곳에 비누 설치 · 밟은 상대는 진행하던 방향으로 2초 동안 길게 미끄러짐 · 미끄러짐이 끝나면 피해 30 · 피해를 받은 뒤 1.5초 기절 · 내가 설치한 비누에는 미끄러지지 않음",
    );
