...
describe("content description SSOT", () => {
  it("derives every item description from its definition", () => {
    const expected = {
      ...
      soap: "5회 · 내 위치에서 최대 3칸 떨어진 곳에 비누 설치 · 밟은 상대는 2초 동안 미끄러짐 · 미끄러짐이 끝나면 피해 25 · 피해를 받은 뒤 2초 기절 · 내가 설치한 비누에는 미끄러지지 않음",
      "brick-bag":
      ...
      bomb: "2회 · 내 위치에서 최대 0.75칸 떨어진 곳에 폭탄 설치 · 3.25초 뒤 폭발 · 폭발 반경 3칸 · 피해 65 · 설치자는 피해의 20%를 받음",
    } as const;
  ...
  it("separates card metadata from the effect copy without duplicating charges", () => {
    const soap = getItemDefinition("soap");
    expect(formatItemDescription(soap)).toMatch(/^5회 · /u);
    expect(formatItemEffectDescription(soap)).toBe(
      "내 위치에서 최대 3칸 떨어진 곳에 비누 설치 · 밟은 상대는 2초 동안 미끄러짐 · 미끄러짐이 끝나면 피해 25 · 피해를 받은 뒤 2초 기절 · 내가 설치한 비누에는 미끄러지지 않음",
    );
    expect(formatItemEffectDescription(soap)).not.toContain("5회");
  });
