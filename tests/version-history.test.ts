...
describe("version history", () => {
  it("keeps the current product version first and records concise reasons and changes", () => {
    ...
    expect(VERSION_HISTORY[0]).toMatchObject({
      version: "0.199.59",
      title: expect.stringContaining("기록된다요"),
      change: expect.stringContaining("내 처치"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.57")).toMatchObject({
      version: "0.199.57",
      title: expect.stringContaining("한눈에 들어온다요"),
      change: expect.stringContaining("두 줄"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.55")).toMatchObject({
      version: "0.199.55",
      title: expect.stringContaining("한눈에 들어온다요"),
    ...
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.53")).toMatchObject({
      version: "0.199.53",
      title: expect.stringContaining("한눈에 들어온다요"),
      change: expect.stringContaining("맨 위부터"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.52")).toMatchObject({
      version: "0.199.52",
      title: expect.stringContaining("삐져나오지 않는다요"),
      change: expect.stringContaining("줄바꿈"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.51")).toMatchObject({
      version: "0.199.51",
      title: expect.stringContaining("보여준다요"),
      change: expect.stringContaining("한눈에 보여주고"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.50")).toMatchObject({
      version: "0.199.50",
      title: expect.stringContaining("보인다요"),
      change: expect.stringContaining("포인트당 효과"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.49")).toMatchObject({
      version: "0.199.49",
      title: expect.stringContaining("안 잘린다요"),
      change: expect.stringContaining("세로로 쌓아"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.48")).toMatchObject({
      version: "0.199.48",
      title: expect.stringContaining("안 잘린다요"),
      change: expect.stringContaining("전폭을 쓰게"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.47")).toMatchObject({
      version: "0.199.47",
      title: expect.stringContaining("한 장에"),
      change: expect.stringContaining("안 삐져나오게"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.46")).toMatchObject({
      version: "0.199.46",
      title: expect.stringContaining("안 잘린다요"),
      change: expect.stringContaining("셀 안에"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.45")).toMatchObject({
      version: "0.199.45",
      title: expect.stringContaining("카드 안에"),
      change: expect.stringContaining("안 겹치고"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.44")).toMatchObject({
      version: "0.199.44",
      title: expect.stringContaining("일시정지"),
      change: expect.stringContaining("한 장"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.43")).toMatchObject({
      version: "0.199.43",
      title: expect.stringContaining("한 번에"),
      change: expect.stringContaining("MAX"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.42")).toMatchObject({
      version: "0.199.42",
      title: expect.stringContaining("완료 통계"),
      change: expect.stringContaining("한 화면"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.41")).toMatchObject({
      version: "0.199.41",
      title: expect.stringContaining("점수표"),
      change: expect.stringContaining("한 줄"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.35")).toMatchObject({
      version: "0.199.35",
      title: expect.stringContaining("커버"),
      change: expect.stringContaining("가로 스크롤"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.31")).toMatchObject({
      version: "0.199.31",
      title: expect.stringContaining("다섯"),
      change: expect.stringContaining("두 칸"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.30")).toMatchObject({
      version: "0.199.30",
      title: expect.stringContaining("조작"),
      change: expect.stringContaining("오른쪽 위"),
    });
    expect(VERSION_HISTORY.find(({ version }) => version === "0.199.29")).toMatchObject({
      version: "0.199.29",
