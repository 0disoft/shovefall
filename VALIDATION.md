...
| `audit-rounds` | Bun deterministic round audit | Runs sixteen fixed-seed all-bot rounds each at 4, 12, 24, and 32 participants, emits raw terminal observations and duration percentiles, and fails on malformed results or a time-limit draw. It is not human-play balance evidence. |
...
Web-app validation covers the playable DOM/PixiJS gray-box, deterministic utility bots, seeded collapse, three bounded items, item settings, pre-round countdown, round result, fresh-world restart, optional procedural audio, reduced-motion presentation, renderer loss recovery, spatial broad phase, local production-artifact smoke, and bounded local scale profiles without claiming hosted deployment, physical-device, cross-browser, audio-device, or external playtest approval.
