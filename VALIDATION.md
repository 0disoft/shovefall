...
| `test` | Vitest 4 | Runs loadout normalization, starting effects, deterministic connected island and lake generation, supported spawn placement, protected 20% collapse cores across every participant tier, replay parsing, spatial-hash equivalence, slower mass-sensitive movement, compact hand-reach shove timing, batch impulse, dodge, support, credited elimination, bounded stat spending, item effects, collapse, round sealing, report v3, keyboard/pointer/gamepad input state, version-history completeness, 58-degree projection math and camera bounds, bot determinism, shoreline safety, delayed perception, tile safety, and targeting coverage. |
...
| `profile-scale` | Bun hard-difficulty headless local profile | Measures 7,200 ticks each at 16, 24, and 32 participants, separating AI and simulation duration while reporting candidate/full-pair source counts, long steps, and observational heap deltas. The enlarged-island simulation p95 budgets are 4/7/7 ms, leaving browser-frame headroom rather than preserving the smaller-map budget by assertion. |
...
| `audit:items` | Bun deterministic starting-loadout balance audit | Runs at least sixty-four fixed-seed 8-participant rounds with Fast collapse, a 45-second audit limit, and production item spawns disabled. The sample count grows to keep at least sixteen actor-round slots per treatment as the catalog expands. Normal-mass scripted actors rotate independently through bot personalities, the no-item control, every single starting item, and every unique two-item pair. It reports wins, 95% Wilson intervals, no-item-relative win indices, credited eliminations, mean survival time, pair independence expectations, synergy indices, duration bounds, and time-limit seeds without dumping every round record. It fails only on malformed/incomplete sampling; buff and nerf labels are review signals rather than automated balance verdicts. It is a fast tuning screen, not human-play, shoreline-pickup, other-mass, other-count, or active-item-use evidence. |
...
Web-app validation covers the playable DOM/PixiJS gray-box, deterministic connected island coastlines and lakes, a connected 20% collapse floor, deterministic Easy/Normal/Hard utility bots, selectable Slow/Normal/Fast seeded collapse, three bounded shoreline-weighted items, 8/16/24/32 presets, item settings, pre-round countdown, round result, fresh-world restart, optional procedural audio, reduced-motion presentation, renderer loss recovery, spatial broad phase, local production-artifact smoke, deterministic mass, synthetic item-grant, and collapse-pacing screens, and hard-difficulty local scale profiles without claiming hosted deployment, physical-device, cross-browser, audio-device, risky-pickup item balance, human balance, or external playtest approval.
