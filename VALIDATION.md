...
| `test` | Vitest 4 | Runs loadout normalization, starting effects, deterministic connected island and multi-lake generation, supported spawn placement, the 50-participant arena budget, protected 20% collapse cores across every internal tier, replay parsing, spatial-hash equivalence, numeric weight mapping, slower mass-sensitive movement, compact hand-reach shove timing, batch impulse, dodge, support, credited elimination, bounded stat spending, item effects, collapse, round sealing, report v4, keyboard/pointer/gamepad input state, version-history completeness, 58-degree projection math and camera bounds, bot determinism, shoreline safety, delayed perception, tile safety, and targeting coverage. |
...
Web-app validation covers the playable DOM/PixiJS gray-box, deterministic connected island coastlines and multiple lakes, a connected 20% collapse floor, one public 50-participant Hard-AI mode, a 50–100 starting-weight slider, selectable Slow/Normal/Fast seeded collapse, three bounded shoreline-weighted map items, the nine-entry starting-item catalog, item settings, pre-round countdown, round result, fresh-world restart, optional procedural audio, reduced-motion presentation, renderer loss recovery, spatial broad phase, local production-artifact smoke, deterministic mass, synthetic item-grant and collapse-pacing screens, and 50-participant local profiles without claiming hosted deployment, physical-device, cross-browser, audio-device, risky-pickup item balance, human balance, or external playtest approval.
