...
| `smoke-dist` | Vite production build plus Playwright Test | Runs the twelve production-safe browser smoke paths against `vite preview` of the generated `dist` artifact. It excludes only the explicitly tagged development-only fatal injector and does not claim hosted deployment evidence. |
| `profile-scale` | Bun hard-difficulty headless local profile | Measures 7,200 ticks at the public 50-participant setting with deterministic Brick Bag, two-Bomb, and Soap placement, separating AI and simulation duration while reporting wall/Bomb/Soap samples, simultaneous detonations, observed Soap triggers, candidate/full-pair source counts, long steps, real-time throughput, and observational heap delta. Soap trigger counts remain descriptive rather than a merge gate; the simulation p95 gate is 10 ms, and AI timing remains visible independently because it is staggered rather than charged every simulation tick. |
...
| `profile-browser` | Production build plus local headless Chrome | Measures a fixed hard-difficulty Brick Bag plus Bomb seed at 1280×720 across the complete five-second fuse and detonation for frame p95, long frames, requested-versus-delivered ticks, backlog, effective DPR, and forced-GC heap change after 20 restarts. It is not cross-browser or field evidence. |
...
Web-app validation covers the playable DOM/PixiJS gray-box, deterministic connected island coastlines and multiple lakes, a connected 20% collapse floor, one public 50-participant Hard-AI mode, a 50–100 starting-weight slider, selectable Slow/Normal/Fast seeded collapse, three bounded shoreline-weighted map items, the nine-entry catalog, eight currently offered loadout items, two-charge Wind Blast, four-charge Brick Bag, one-charge five-second Boat, two-charge five-second Bomb, and three-charge Soap through keyboard/gamepad/DOM slot bridges, static-wall attack and body collision, bounded water support, deterministic radial blast and one-use slip-patch state, item settings, pre-round countdown, round result, fresh-world restart, optional procedural audio, reduced-motion presentation, renderer loss recovery, spatial broad phase, local production-artifact smoke, deterministic mass, synthetic item-grant and collapse-pacing screens, and 50-participant local profiles without claiming hosted deployment, physical-device, cross-browser, audio-device, active-item bot balance, risky-pickup item balance, human balance, or external playtest approval.
