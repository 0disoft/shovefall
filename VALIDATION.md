...
| `test` | Vitest 4 | Runs settings with bounded bot difficulty and collapse-speed normalization, deterministic simulation, replay difficulty parsing, spatial-hash boundary and full-scan equivalence, mass, action timing, batch impulse, dodge, support, falling, triple-overlap containment, a 21-case swept grazing-contact matrix, item stacking and exact expiry, boosted multi-target shove, seeded item placement and caps, collapse phases and ordering, round sealing, time-limit draws, local playtest report completion guards and serialization, input state, bot command shape, bot determinism, delayed perception, tile and edge safety, and identity-neutral targeting coverage. |
