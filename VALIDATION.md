...
| `smoke-dist` | Vite production build plus Playwright Test | Runs the eleven production-safe browser smoke paths against `vite preview` of the generated `dist` artifact. It excludes only the explicitly tagged development-only fatal injector and does not claim hosted deployment evidence. |
| `profile-scale` | Bun hard-difficulty headless local profile | Measures 7,200 ticks at the public 50-participant setting with Brick Bag placement and Bomb placement/detonation, separating AI and simulation duration while reporting wall/Bomb samples, simultaneous detonations, candidate/full-pair source counts, long steps, real-time throughput, and observational heap delta. The simulation p95 gate is 10 ms; AI timing remains visible independently because it is staggered rather than charged every simulation tick. |
...
Web-app validation covers the playable DOM/PixiJS gray-box, deterministic connected island coastlines and multiple lakes, a connected 20% collapse floor, one public 50-participant Hard-AI mode, a 50–100 starting-weight slider, selectable Slow/Normal/Fast seeded collapse, three bounded shoreline-weighted map items, the nine-entry catalog, seven currently offered loadout items, two-charge Wind Blast, four-charge Brick Bag, one-charge five-second Boat, and two-charge five-second Bomb through keyboard/gamepad/DOM slot bridges, static-wall attack and body collision, bounded water support, deterministic radial blast state, item settings, pre-round countdown, round result, fresh-world restart, optional procedural audio, reduced-motion presentation, renderer loss recovery, spatial broad phase, local production-artifact smoke, deterministic mass, synthetic item-grant and collapse-pacing screens, and 50-participant local profiles without claiming hosted deployment, physical-device, cross-browser, audio-device, active-item bot balance, risky-pickup item balance, human balance, or external playtest approval.
