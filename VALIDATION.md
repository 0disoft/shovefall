...
| `audit-rounds` | Bun deterministic round and balance audit | Runs sixteen fixed-seed production-preset rounds each at 8, 16, 24, and 32 participants, records duration, terminal results, item spawn bands, observational item exposure, and twenty-four controlled 16-participant base-mass rounds. It fails on malformed results, time-limit draws, a missing outer-ring preference, or an extreme controlled mass skew. It is not human-play or causal item-balance evidence. |
...
Web-app validation covers the playable DOM/PixiJS gray-box, deterministic utility bots, seeded collapse, three bounded edge-weighted items, 8/16/24/32 presets, item settings, pre-round countdown, round result, fresh-world restart, optional procedural audio, reduced-motion presentation, renderer loss recovery, spatial broad phase, local production-artifact smoke, deterministic balance screening, and bounded local scale profiles without claiming hosted deployment, physical-device, cross-browser, audio-device, causal item, human balance, or external playtest approval.
