...
| `profile-browser` | Production build plus local headless Chrome | Measures fixed hard-difficulty Brick-plus-Bomb and Grappling-Hook-plus-Bomb workloads at 1280×720 after a separate startup warmup and same-arena stabilization. Both conditions report frame p95, long frames, requested-versus-delivered ticks, backlog, effective DPR, and forced-GC heap change after 20 restarts. The Hook condition spends both charges before its result is accepted. It is not cross-browser or field evidence, and a host-contended observation cannot be relabeled as a pass. |
