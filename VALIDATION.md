...
| `smoke-dist` | Vite production build plus Playwright Test | Runs thirteen production-safe browser paths against `vite preview` of the generated `dist` artifact, including Grappling Hook selection, charge use, feedback, and a live arena. It excludes only explicitly tagged development-only injectors and does not claim hosted deployment evidence. |
