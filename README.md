...
The toolchain, replay format v2, fixed-tick physics, swept-circle and static-wall contact, browser scheduler, pausable countdown, keyboard adapter, Coal-Twilight PixiJS renderer, fixed Hard utility bots, selectable Slow/Normal/Fast collapse, results, spatial broad phase, one 50-participant browser mode, bounded edge-weighted item settings, three passives, Wind Blast, Brick Bag, Boat, Bomb, Soap, accelerated defeat resolution, restart, optional procedural audio, reduced motion, renderer recovery, deterministic round/balance/pacing auditing, and source-owned GitHub Actions Pages workflow are implemented. The local `0.31.0` suite passes 160 unit/scenario tests and twelve production-safe Chrome paths. With Brick Bag, two simultaneous Bombs, and three Soap patches observed, the fixed-50 headless profile reaches simulation p95 `6.265 ms`, zero 100 ms steps, five Soap triggers, and `2.35×` real time. The current-renderer local Chrome rerun reaches frame p95 `18.4 ms` with zero backlog after one rejected host-contended sample; the production-safe Soap path separately proves live placement and feedback rather than a dedicated live-Soap frame window. CI run `30030306125` remains the exact hosted proof for `v0.30.0`; `0.31.0` requires a fresh run and public smoke. Bot use and balance evidence for active items, the remaining Grappling Hook mechanic, branch protection, broader physical-device and cross-browser coverage, external playtest, final captures, and any optional external image asset inventory remain pending or separately gated.
