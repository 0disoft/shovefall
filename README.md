...
The toolchain, replay format v2, fixed-tick physics, swept-circle and static-wall contact, browser scheduler, Coal-Twilight PixiJS renderer, fixed Hard utility bots, selectable collapse speed, automatic growth planning, cannon collapse, protected-core rock pressure, one 50-participant browser mode, nine starting items, two accepted generated-image atlases, optional procedural audio, deterministic balance auditing, and source-owned GitHub Actions Pages workflow are implemented locally for product `0.35.0`, simulation `18.0.0`, and content `11.0.0`; reports use v5. Production HTML contains no developer telemetry or tuning-lab markup. The current unit/scenario suite has 184 tests; browser, balance, performance, hosted, and exact-SHA capture evidence must be refreshed before `0.35.0` replaces the deployed `0.34.1` candidate. Bot use and human balance for active items, uncontended browser performance, broader physical-device and cross-browser coverage, six remaining generated-image packs, and external playtest remain pending or separately gated.
