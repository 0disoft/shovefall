...
The repository contains a playable semantic DOM and PixiJS WebGL client, exact package graph, local validation commands, and a Clarissimi-backed public contribution review path. A pausable `3→2→1` session countdown renders the fresh arena while keeping simulation, bots, and input at tick zero. Public play is one 50-participant Hard-AI mode; the human chooses a numeric starting weight from 50 through 100, two distinct items from nine offered choices, and a pre-round automatic growth order. WASD, arrow keys, mouse or touch drag, a touch joystick, a standard gamepad stick, or its D-pad starts, turns, and stops ordinary movement immediately; Space or the first action button extends a short hand shove without launching the attacker; Shift or the second action button dodges; and Q/E or the remaining face/arena buttons use inventory slots. Elimination-earned points follow the saved growth order automatically instead of interrupting combat. Wind Blast launches the first unshielded body on its ray. Brick Bag places four cardinal walls, and a dodge that reaches one mounts it as an immovable non-attacking perch until movement or flooding dismounts the rider. Boat grants five seconds of movement across in-arena water. Bomb leaves a visible five-second hazard that directly eliminates every body in its three-tile radius, including its owner; mass, Dodge, and Brick cannot save a target. Soap leaves a one-use slip patch. `구조 갈고리` pulls only its user toward current land or the first Brick wall on its ray. The public 48×40 procedural island contains exactly eight separated lakes and a connected 20% protected core. Eight offshore pirate ships hold exactly one cannon round per doomed tile, expose orange and red trajectory warnings, and fill each impact tile with water; after ammunition runs out, targeted lethal rocks pressure survivors without deleting the core. Item placement selects edge, near-edge, or interior at a stable 3:2:1 ratio. Owner-generated character and item atlases load asynchronously from the same origin and retain procedural fallbacks. Product `0.35.0`, simulation `18.0.0`, content `11.0.0`, and local reports v5 own these current local rules. The newest accepted hosted proof is still product `0.34.1`: [CI run `30061893140`](https://github.com/0disoft/shovefall/actions/runs/30061893140) validated, captured, and deployed exact runtime SHA `354a602392cccb453ebb1a4ac1fd52c5a39fac6c`. Hosted `0.35.0`, human playtest, uncontended browser performance, and cross-browser checks remain pending.
...
The toolchain, replay format v2, fixed-tick physics, swept-circle and static-wall contact, browser scheduler, Coal-Twilight PixiJS renderer, fixed Hard utility bots, selectable collapse speed, automatic growth planning, cannon collapse, protected-core rock pressure, one 50-participant browser mode, nine starting items, seven accepted generated-image assets, optional procedural audio, deterministic balance auditing, and source-owned GitHub Actions Pages workflow are implemented locally for product `0.35.0`, simulation `18.0.0`, and content `11.0.0`; reports use v5. Production HTML contains no developer telemetry or tuning-lab markup. The current unit/scenario suite has 184 tests and the production artifact passes thirteen Chrome paths. Hosted and exact-SHA capture evidence must still refresh before `0.35.0` replaces the deployed `0.34.1` candidate. Bot use and human balance for active items, uncontended browser performance, broader physical-device and cross-browser coverage, remaining terrain/action/prop art, and external playtest remain pending or separately gated.
