...
The toolchain, deterministic replay, gray-box physics, swept-circle contact, fixed-step browser scheduler, pausable countdown, keyboard adapter, procedural PixiJS renderer, selectable Easy/Normal/Hard utility bots, collapse, results, spatial broad phase, 8/16/24/32 browser presets, bounded edge-weighted item settings, Iron Boots, Feather, Spring Glove, accelerated defeat resolution, restart, optional procedural audio, reduced motion, renderer recovery, deterministic round/balance auditing, and source-owned GitHub Actions workflow are implemented. Development-server and generated-`dist` Chrome smoke pass, and hard-difficulty item-enabled headless and production-Chrome profiles pass at 16/24/32 with continuous contact enabled. Hosted workflow status, branch protection, physical-device, cross-browser, risky-pickup item balance, external playtest, static hosting, final visual direction, and the image asset inventory remain pending or separately undecided.
