...
The toolchain, replay format v2, fixed-tick physics, swept-circle contact, browser scheduler, pausable countdown, keyboard adapter, Coal-Twilight PixiJS renderer, fixed Hard utility bots, selectable Slow/Normal/Fast collapse, results, spatial broad phase, one 50-participant browser mode, bounded edge-weighted item settings, three passives, two-charge Wind Blast, accelerated defeat resolution, restart, optional procedural audio, reduced motion, renderer recovery, deterministic round/balance/pacing auditing, and source-owned GitHub Actions Pages workflow are implemented. Generated-`dist` Chrome smoke and the local 50-participant Hard profile must be refreshed for `0.27.0`. Bot use and balance evidence for active items, the other five active mechanics, branch protection, broader physical-device and cross-browser coverage, external playtest, final captures, and any optional external image asset inventory remain pending or separately gated.
