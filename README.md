...
The toolchain, deterministic replay, gray-box physics, swept-circle contact, fixed-step browser scheduler, keyboard adapter, procedural PixiJS renderer, utility bots, collapse, results, spatial broad phase, 4–32 participant presets, bounded item settings, Iron Boots, Feather, Spring Glove, accelerated defeat resolution, restart, optional procedural audio, reduced motion, and renderer recovery are implemented. Item-enabled headless and production-Chrome profiles pass at 12/24/32 with continuous contact enabled. Physical-device, cross-browser, external playtest, static hosting, final visual direction, and the image asset inventory remain pending or separately undecided.
