...
`GameConfigV1` is normalized before world creation. It supports 4 through 50 participants, arenas from 7 through 48 columns and rows, a maximum 120-second replay horizon, slow, normal, or fast collapse, normal density, Easy/Normal/Hard bot difficulty, item policy version 2, a bounded initial item count, a participant-derived simultaneous cap, and a zero-to-thirty-second spawn interval. The same raw values normalize identically, and disabled items require zero initial and spawn values. Browser settings force 50 participants and Hard difficulty; smaller counts and other difficulty values remain simulation fixtures and diagnostic inputs. Browser-local `GameSettings` additionally owns the ordered automatic upgrade plan because it selects future explicit commands rather than changing `GameConfigV1` world construction.
