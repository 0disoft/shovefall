...
`GameConfigV1` is normalized before world creation. It supports 4 through 50 participants, arenas from 7 through 48 columns and rows, a maximum 120-second replay horizon, slow, normal, or fast collapse, normal density, Easy/Normal/Hard bot difficulty, item policy version 2, a bounded initial item count, a participant-derived simultaneous cap, and a zero-to-thirty-second spawn interval. The same raw values normalize identically, and disabled items require zero initial and spawn values. Browser settings force 50 participants, Hard difficulty, Slow collapse, eight initial map items, one spawn every four seconds, and a 120-second round limit; smaller counts and other values remain simulation fixtures and diagnostic inputs. Browser-local `GameSettings` owns only the starting weight and two-item loadout. A credited human elimination pauses the browser scheduler until one valid stat command is saved; inactive, Falling, Eliminated, and completed states cannot request or accept that choice.
...
A participant contains stable identity, human or scripted control ownership, an active flag, a circular body, two-slot starting inventory, temporary effects, progression, offensive credit, and an action state. Inventory slots preserve their index, item definition, and nullable charge count; `null` means a permanent passive. The body owns position, velocity, facing, radius, base and effective continuous `massFactor`, and integer unsupported ticks. Effective mass is clamped to `0.85..1.25`. Progression owns unspent points, credited eliminations, and bounded Power, Stability, Mobility, and Reflex levels. The compatibility-named `shoveCredit` state records attacker, hit tick, and impulse strength. Newer hits replace older hits; same-tick Wind Blast and shove claims choose greater strength, then lower attacker ID. Falling grants one point only when that hit is at most 180 ticks old; Bomb and eligible rock eliminations use the same canonical award path without creating a fake fall.
...
World construction creates eight immutable pirate-ship positions around the arena. Every collapse-plan tile becomes one `CannonShotState`; round-robin assignment makes the sum of initial ship ammunition equal the exact doomed-tile count and keeps the largest and smallest ship totals within one round. Cannon launch precedes the tile warning, the orange warning begins around the flight midpoint, the red danger stage follows, and impact shares the authoritative `Void` tick. Ship ammunition is derived from sorted launch ticks rather than mutable state, and active cannon shots are presentation facts rather than collision entities.
...
- `RenderFrameV1` is an immutable presentation snapshot with current and previous positions, facing, mass, inventory slots, effects, Spring Glove telegraph, map items, brick walls, armed bombs, pirate ships, active cannon and rock shots, action, tiles, tick, and state hash.
