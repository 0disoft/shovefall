...
Product `0.198.0` moves the public workload to 70 participants on a 57×48 arena. The headless and production-browser profiles now target that workload without weakening the 10 ms simulation p95, 25 ms browser frame p95, or tick-delivery gates. No fresh 70-participant profile has been recorded yet, so the 60-participant measurements below remain historical failure evidence rather than proof for the current build.
...
On 2026-07-24, the 7,200-tick fixed-50 profile fixed actor 1 at a supported central tile, placed one Brick wall and up to three Soap patches, and forced actors 2 and 3 to place two Bombs on the same tick. It observed one wall with `0.958` mean walls per tick, a peak of two Bombs with `0.167` mean bombs per tick, a peak of three Soap patches with `0.242` mean patches per tick, and five Soap triggers. It completed in `51,033.242 ms`, or `2.35×` real time. AI p95 was `9.829 ms`, simulation p95 was `6.265 ms`, the spatial candidate/full-pair ratio was `0.1815`, and no combined step exceeded 100 ms. The `20,787,755`-byte heap delta remains observational because the harness does not force garbage collection. Soap trigger count is reported but is not a merge gate.

The production-artifact Chrome smoke selects Soap, starts a fresh fixed-50 round, places a patch, observes its charge fall from three to two, and confirms the dedicated feedback. The `0.31.0` production-Chrome regression profile at 1280×720 and effective DPR 1 measured frame p95 `18.4 ms`, maximum `18.8 ms`, `61.58` delivered ticks per requested simulation second, zero backlog, no frame above 100 ms, and a `2,923,628`-byte used-heap delta after twenty immediate restarts and CDP garbage collection. An immediately preceding host-contended sample failed with frame p95 `71.5 ms`, backlog `918`, and a `6×` maximum simulation rate; the clean rerun is accepted without hiding the rejected observation. The fixed browser fixture exercises Brick Bag and Bomb through the current renderer but does not retain a live Soap patch during its sampled frame window, so the production-safe Soap smoke remains the live-Soap presentation proof. These are local workloads, not physical-device, cross-browser, or field evidence.
