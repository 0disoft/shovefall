...
The PixiJS renderer derives short-lived shove, dodge, fall, item, water-impact, and result effects from accepted events. It also derives timed-effect, Spring Glove, mounted-wall, offshore ship, cannon-arc, orange exclamation, and red skull markers from the current render frame. Sixteen measured character frames and nine measured item frames are cut from two owner-generated transparent atlases and cached as PixiJS textures. Loaded character art keeps only the human foot-level chevron and explicit item badges instead of overlapping mass and control-state glyphs. Generated cannonball art replaces its procedural circle rather than layering over it; a rejected asset load restores the procedural fallback. Loading is asynchronous and does not delay countdown or simulation. Normal rounds cap transient effects at 36. The fixed 50-participant mode caps them at 14 and removes nonessential bot dodge trails while keeping human feedback and authoritative artillery telegraphs. Eight ship labels and generated sprites are cached and updated instead of recreated every frame.
...
A red vignette flash overlays the arena when the human takes damage. Camera shake is not used: Bombs, cannon impacts, skill hits, and eliminations never modify camera coordinates. The vignette is suppressed under reduced motion and never enters the simulation or replay hash.

Arc Bolt and Chain Bind emit a deterministic launch event, travel at six tiles per second, and apply their authoritative hit when the projectile tip first reaches the target body's outer edge. The renderer keeps every in-flight skill projectile outside the transient-effect cap, follows a moving target, and removes it on the same tick as the generated impact cue, damage, and control. Reduced motion keeps the same flight and impact timing while suppressing only nonessential transforms.
