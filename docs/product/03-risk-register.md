...
| R-08 | Local Chrome evidence is mistaken for physical-device, cross-browser, hosted-CI, or deployed-site proof. | Medium | High | Validation documents name each proof boundary. [CI run 30087368621](https://github.com/0disoft/shovefall/actions/runs/30087368621) validated thirteen production Chrome paths and deployed exact `0.37.0` SHA `7386349b43a6e83bd0071873184f70e303c92ef0`; a cache-busted public HTTP check returned the new application bundle. Physical-device, cross-browser, and `0.38.0` hosted proof remain absent. | Before submission, capture a named-device browser matrix and final captures against the same runtime candidate; do not substitute local or hosted Chrome for those boundaries. | Open |
...
| R-11 | CPU-heavy profiling and timing-sensitive E2E contend when run in parallel, producing input-hold and timeout flakes. | Medium | Medium | Gates are documented to run serially; the held-increment test (bootstrap.spec.ts:1477) failed only while profile:scale ran concurrently and passed alone in 4.1s. capture-submission's 20s waitForTickDelta is a separate known flake. | Never run profile:scale concurrently with Playwright; rerun any timed-out input test alone before treating it as a regression. | Monitoring |
| R-12 | The headless profile gate's absolute p95 threshold (<=10ms) is machine-dependent and fails on this workstation regardless of code change. | High | Low | Baseline (c467d09): simulation p95 21.1ms, longSteps 0. After the constructor-only perf commits (f20211b, 09261c1): p95 23.9ms, longSteps 0; step() code and the broad-phase candidate ratio (0.1184) are unchanged, so the delta is measurement noise. | Compare relative to baseline; keep the threshold unchanged; treat Worker offload (next wave) as the real fix. | Open |
