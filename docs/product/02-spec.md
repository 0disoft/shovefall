...
Version `0.9.0` adds a deduplicated presentation-event boundary, bounded procedural Web Audio with an explicit mute control and silence fallback, reduced-motion effects, Mayhem-specific effect caps, deterministic human-defeat recovery coverage, and WebGL context-loss pause and restoration. These changes consume simulation events without changing authoritative rules, so simulation `4.0.0` and content `3.0.0` remain compatible. Replay fixtures change only to record the product version. Item-enabled headless and production Chrome pass the 12/24/32 local budgets. Physical-device, cross-browser, round-duration, balance, final-art readability, audio-device, and external playtest approval remain pending.
