...
Version `0.8.0` promotes Iron Boots, Feather, and Spring Glove as deterministic content. Initial count and respawn interval are bounded settings; simultaneous capacity derives from participants and remaining stable area. Duplicate timed effects refresh, opposing mass effects coexist within global bounds, and one Spring Glove powers every valid target in its next shove window before being consumed. The setup and PixiJS layers expose item state without owning it. Simulation `4.0.0`, content `3.0.0`, and replay fixtures intentionally change. Item-enabled production Chrome passes the 12/24/32 local budgets. The final headless tail rerun after allocation optimization, physical-device, cross-browser, round-duration, balance, readability, and external playtest approval remain pending.
