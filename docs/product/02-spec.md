...
Version `0.10.0` closes maximum-speed grazing tunneling by resolving the analytic first swept-circle contact before overlap correction. A 21-case deterministic geometry matrix and triple-overlap containment test protect the boundary. Because previous position now changes collision outcomes, simulation advances to `5.0.0`; content remains `3.0.0`, and replay fixtures are regenerated intentionally. Headless and production Chrome pass the 12/24/32 local budgets with continuous contact enabled. The `0.9.0` presentation-event, optional-audio, reduced-motion, Mayhem-cap, defeat-recovery, fatal-recovery, and WebGL-restoration behavior remains active. Physical-device, cross-browser, round-duration, balance, final-art readability, audio-device, and external playtest approval remain pending.
