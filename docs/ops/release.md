...
Product `0.36.0` keeps simulation `18.0.0` and advances content to `12.0.0`. Forty-nine public bots receive a seed-derived balanced passive-plus-active loadout, then request charged items through the existing command contract after delayed perception, context-specific utility checks, and a deterministic decision cooldown. Generated character motion, placed Bomb and Soap props, Boat presentation, and bounded camera kick remain presentation-only. Formatting and TypeScript checks pass; 187 unit/scenario checks include a 50-participant 30-second active-item screen. Production Chrome, full round audits, profile, capture, hosted CI, Pages, and public functional evidence remain grouped promotion gates.
