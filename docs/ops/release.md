...
Product `0.199.59`, simulation `84.1.0`, and content `50.0.0` keep replay v8 and reports v11. A soap slip now records an offensive credit at trigger time, so a victim who slides off the arena is attributed to the soap owner and the kill-reward trait point is awarded. Replay fixtures were regenerated for the new simulation version; recorded hashes and checkpoints are unchanged. Hosted proof and physical-device validation remain pending.
