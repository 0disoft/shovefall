...
- Status: `0.199.59` soap-kill attribution candidate; hosted proof pending
- Primary owner: Repository owner
- Current product version: `0.199.59`

Product `0.199.59`, simulation `84.1.0`, and content `50.0.0` keep replay v8 and reports v11. A soap slip now records an offensive credit at trigger time, so a victim who slides off the arena is attributed to the soap owner and the kill-reward trait point is awarded. The native browser context menu is suppressed on every game screen — menu, settings, version history, scoreboard, the active arena, the in-game overlays, and the completed or dead round panel — so right-click stays a game-owned input everywhere. Post-elimination spectator catch-up now runs at 3× instead of 6×. Replay fixtures were regenerated for the new simulation version; recorded hashes and checkpoints are unchanged. Hosted proof and physical-device validation remain pending.
