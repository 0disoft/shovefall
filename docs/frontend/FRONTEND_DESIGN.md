...
React, Vue, Svelte, Phaser, Tailwind CSS, a general state manager, and a general-purpose physics engine are not part of the initial baseline. The contest-submission visual direction is Coal-Twilight: a single dark palette shared by the DOM shell and the PixiJS canvas. Owner-generated character-variant and item-icon PNG atlases have passed the asset ledger and now layer over the existing procedural renderer. Pirate-ship, cannon, rock, terrain, character-action, and item-prop image packs remain outside the runtime because their first outputs contained a baked checkerboard instead of alpha transparency. Reduced-motion presentation follows the same authoritative telegraph timing without depending on animation amplitude.
...
The toolchain, deterministic simulation, direct gray-box movement, hand-shove combat, browser scheduler, pausable countdown, multi-device adapters, procedural PixiJS presentation, fixed Hard utility bots, selectable Slow/Normal/Fast collapse, automatic growth planning, match-readable HUD, round results, local report v5 copy, accelerated defeat resolution, restart, spatial broad phase, one 50-participant setup mode on a 48×40 island with eight separated lakes, topology-independent 3:2:1 item risk bands, all nine starting items, Brick dodge mounting, direct-kill Bombs, eight-ship cannon collapse, protected-core lethal rock pressure, two asynchronously loaded generated atlases, optional procedural audio, reduced-motion effects, renderer recovery, and a production-free developer telemetry boundary are implemented for product `0.35.0`. At 25 or more participants, generated character variants layer over mass-scaled procedural identity, action, facing, and stumble/fall signals; text-backed settings cards and procedural pickup geometry remain usable when art fails. Hosted `0.35.0` proof, the six remaining image packs, bot active-item use, human active-item balance, and final-art readability remain gated on their own evidence.
