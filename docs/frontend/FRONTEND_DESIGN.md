...
Vitest covers pure state, keyboard and pointer vectors, gamepad dead zones and slot edges, loadout normalization, starting effects, charged use, first-hit targeting, Brick Bag placement and rejection, same-tick wall shielding, swept wall stops, hand-shove corner occlusion, Void-tile wall removal, Boat activation at the support deadline, exact five-second expiry, out-of-arena rejection, same-tick Wind vulnerability, dodge and command priority, launch transfer, same-tick credit arbitration, replayed setup, compact hand reach, credited elimination, bounded stat spending, settings tiers, collapse phases, result sealing, version-history/product-version alignment, 58-degree projection math, projected camera bounds, and presentation boundaries. Playwright covers version-history entry and focus return, setup, debug tuning copy/apply, countdown, keyboard, Q/E Wind, Brick Bag, and Boat charge use, arrow-key, mouse-drag and narrow-viewport joystick movement, touch action bridging, deterministic human defeat and restart, audio fallback, reduced motion, fatal recovery, and WebGL restoration. Physical touch hardware and gamepad hardware remain manual device-matrix gates.

## 15. Implementation Sequence

The toolchain, deterministic simulation, gray-box movement and combat, browser scheduler, pausable countdown, keyboard adapter, procedural PixiJS presentation, fixed Hard utility bots, selectable Slow/Normal/Fast collapse, readable collapse states, round results, local playtest-record copy, accelerated defeat resolution, restart, spatial broad phase, one 50-participant setup mode, local 50-participant profiles, production-artifact smoke, three procedural map-item markers, a nine-entry starting-item catalog, effect HUD, bounded edge-weighted item settings, Wind Blast, Brick Bag, Boat, Bomb, and Soap controls and feedback, depth-sorted static walls, bounded in-arena water support, deterministic five-second bomb entities, one-use Soap patches, presentation-event deduplication, optional procedural audio, reduced-motion effects, and renderer recovery are implemented. At 25 or more participants, bot rendering preserves mass-scaled body size, action color, facing, and stumble/fall marks while omitting redundant shadows, rings, and badges; the human retains the full identity treatment. Grappling Hook, bot active-item use, and final art remain gated on their own implementation and user-provided or approved assets.
