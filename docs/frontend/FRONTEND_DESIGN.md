...
The toolchain, deterministic simulation, gray-box movement and combat, browser scheduler, pausable countdown, keyboard adapter, procedural PixiJS presentation, fixed Hard utility bots, selectable Slow/Normal/Fast collapse, readable collapse states, round results, local playtest-record copy, accelerated defeat resolution, restart, spatial broad phase, one 50-participant setup mode, local 50-participant profiles, production-artifact smoke, three procedural map-item markers, a nine-entry starting-item catalog, effect HUD, bounded edge-weighted item settings, Wind Blast, Brick Bag, Boat, and Bomb controls and feedback, depth-sorted static walls, bounded in-arena water support, deterministic five-second bomb entities, presentation-event deduplication, optional procedural audio, reduced-motion effects, and renderer recovery are implemented. At 25 or more participants, bot rendering preserves mass-scaled body size, action color, facing, and stumble/fall marks while omitting redundant shadows, rings, and badges; the human retains the full identity treatment. Soap, Grappling Hook, bot active-item use, and final art remain gated on their own implementation and user-provided or approved assets.
