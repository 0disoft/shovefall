...
The fixed-50 headless harness places one Brick wall, forces two Bombs to share their placement and detonation ticks, and has the human place Soap from a stable central spawn. The 7,200-tick run observed a peak of three Soap patches and five triggers while reporting simulation p95 `6.265 ms`, AI p95 `9.829 ms`, no combined step above 100 ms, and `2.35×` real-time throughput. Trigger count remains descriptive rather than a pass gate. The `0.31.0` local production-Chrome rerun at 1280×720 and effective DPR 1 reports frame p95 `18.4 ms`, maximum `18.8 ms`, zero backlog, no frame above 100 ms, `61.58` delivered ticks per requested simulation second, and a `2,923,628`-byte forced-GC heap delta after twenty restarts. An immediately preceding sample failed the backlog gate with frame p95 `71.5 ms`, backlog `918`, and a `6×` maximum simulation rate while the host was contended; it is retained as a rejected observation rather than averaged into the accepted rerun. The browser fixture exercises the current renderer with Brick Bag and Bomb but does not keep a live Soap patch in the measured frame window. Soap's production-safe Chrome path separately proves selection, placement, charge consumption, renderer boot, and feedback. This passes the declared local budgets but does not replace a dedicated live-Soap frame profile, physical-device, cross-browser, or field evidence.
