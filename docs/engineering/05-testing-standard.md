...
- The Playwright suite covers a menu-only first screen, hidden pre-game canvas, settings save behavior, WebGL initialization after start, non-uniform active-round canvas pixels, player-follow camera frame change, recommended and Mayhem item defaults, keyboard-operable bot difficulty and collapse-speed selection with round propagation, menu-to-settings-to-countdown-to-round transition, countdown tick-zero and input isolation, countdown blur freeze, fixed-tick progress, WASD, arrow-key, mouse-drag and narrow-viewport virtual-joystick movement, touch shove bridging, Space action state, sound mute state, window-blur pause, focus resume, arena focus, menu return, deterministic-clock collapse completion, result focus, fresh-world countdown restart, deterministic human defeat and immediate restart, Web Audio-unavailable silence, reduced motion, injected development fatal recovery, WebGL context-loss pause, and restoration. A mocked PixiJS application separately requires every renderer frame request to call the explicit application presentation boundary and verifies that rightward human movement shifts the world camera left. Physical touch and gamepad hardware remain manual device-matrix evidence.
