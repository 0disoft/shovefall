...
- The Playwright suite covers a menu-only first screen, version-history rendering and Escape focus return, hidden pre-game canvas, one-item/two-skill settings, WebGL initialization after start, full-viewport arena ownership, disabled arena text selection, six direct kill-reward trait choices with a visible save action, non-uniform active-round canvas pixels, player-follow camera movement, `Q/W` skill state, `E` Grappling Hook state, `D` item charges, health and mana HUD, rejected WASD and Shift actions, arrow-key and right-click destination movement, rejected desktop left-drag movement, narrow-viewport virtual-joystick movement, touch-skill bridging, sound mute state, `P` pause persistence across blur/focus, live round movement/combat/control/skill-use statistics, menu/restart flows, deterministic completion and defeat, report v9 copy, Web Audio-unavailable silence, reduced motion, injected development fatal recovery, WebGL context-loss pause, and restoration. A mocked PixiJS application separately requires every renderer frame request to call the explicit application presentation boundary and renders skill/item activation, combat zones, resources, and impact events. Physical touch and gamepad hardware remain manual device-matrix evidence.
- `smoke-dist` builds the static artifact and repeats thirteen production-safe browser flows through `vite preview`. It also asserts that the production DOM contains no developer-panel node. The fatal injector is tagged `@dev-only`; omitting it from production proves that the test hook is absent, not that production fatal recovery was re-executed.
- Separate configured profiles cover 7,200 headless ticks at the public 70-participant setting and fixed-seed production Chrome frame pacing plus 20 immediate restarts. The main round and strategy scenarios use 70 participants; smaller 8- and 16-participant controlled rotations remain diagnostic fixtures for isolating mass, item-slot, and collapse-speed variables and are not public modes. The focused and randomized trait/loadout audits intentionally retain 60 actors because their six equal trait groups and 15 skill-pair by four-item rotation require exactly 60 slots. Profile and balance evidence is not part of the routine smoke suite and does not claim cross-browser, physical-device, risky-pickup item balance, or human-play approval.
