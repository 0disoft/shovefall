...
- The Playwright suite covers browser boot, WebGL initialization, recommended and Mayhem item defaults, setup-to-round transition, fixed-tick progress, WASD movement, Space action state, sound mute state, window-blur pause, focus resume, arena focus, settings return, deterministic-clock collapse completion, result focus, fresh-world restart, deterministic human defeat and immediate restart, Web Audio-unavailable silence, reduced motion, injected fatal recovery, WebGL context-loss pause, and restoration.
...
- Browser promotion must still add readable combat and item-telegraph assertions. Weak-contact triple-overlap and high-speed crossing fuzz remain required before release.
