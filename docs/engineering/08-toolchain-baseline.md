...
- The browser runtime floor is ES2023. The Vite build target, TypeScript `target`, and `lib` are aligned to ES2023, which covers the repository's `Array.prototype.toSorted`, `toReversed`, and `findLast` usage in the simulation and renderer. The effective client floor is Chrome 110+, Edge 110+, Firefox 115+, and Safari 16.4+ (`Array.prototype.toSorted` reached Safari in 16.4). A wider pre-2023 browser floor would require replacing those runtime methods with repository helpers; that is a separate decision, not the current baseline.
